...
EdcaTxopN::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::EdcaTxopN")
    ...
    /* Custom Traces */
    .AddTraceSource ("AccessGranted", "EDCA is granted an access to the channel",
                     MakeTraceSourceAccessor (&EdcaTxopN::m_accessGrantedTrace),
                     "ns3::EdcaTxopN::AccessGrantedTracedCallback")
  ;
  return tid;
}
...
void
EdcaTxopN::SetTxOkCallback (TxPacketOk callback)
{
...
EdcaTxopN::NotifyAccessGranted (void)
{
  ...
  /* New custom trace */
  m_accessGrantedTrace (m_low->GetAddress (), m_queue->GetSize ());
...
//          m_accessAllowed = false;
      ...
      if (m_low->IsTransmissionSuspended ())
        {
//          std::cout << "Resuming Suspended Transmission at " << Simulator::Now ()
//                    << " with remaining period=" << m_remainingDuration << std::endl;
          m_low->UpdateMaximumTransmissionDuration (m_remainingDuration);
          m_low->ResumeTransmission (m_transmissionListener);
          return;
        }
    }

  /* If we did not miss ACK or BLOCK ACK */
...
void
EdcaTxopN::InitiateTransmission (Time cbapDuration)
{
...
EdcaTxopN::MapSrcAddressForAggregation (const WifiMacHeader &hdr)
{
  NS_LOG_FUNCTION (this << &hdr);
  Mac48Address retval;
  if (m_typeOfStation == STA || m_typeOfStation == ADHOC_STA || m_typeOfStation == DMG_STA)
    {
      retval = hdr.GetAddr2 ();
    }
  else
...
EdcaTxopN::MapDestAddressForAggregation (const WifiMacHeader &hdr)
{
  NS_LOG_FUNCTION (this << &hdr);
  Mac48Address retval;
  if (m_typeOfStation == AP || m_typeOfStation == ADHOC_STA || m_typeOfStation == DMG_STA || m_typeOfStation == DMG_AP)
    {
      retval = hdr.GetAddr1 ();
    }
  else
