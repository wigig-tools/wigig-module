...
YansWifiPhy::GetCurrentStandard (void) const
{
  return m_standard;
}
          ...
          if (m_rdsActivated)
            {
              ...
              /* We simply transmit on the channel */
              m_channel->Send (this, packet, GetPowerDbm (txVector.GetTxPowerLevel ()) + m_txGainDb,
...
YansWifiPhy::SendPacket (Ptr<const Packet> packet, WifiTxVector txVector, WifiPreamble preamble, enum mpduType mpdutype)
{
  ...
  NotifyMonitorSniffTx (packet, (uint16_t)GetChannelFrequencyMhz (),
                        GetChannelNumber (), dataRate500KbpsUnits, preamble, txVector, aMpdu);
  m_state->SwitchToTx (txDuration, packet, GetPowerDbm (txVector.GetTxPowerLevel ()), txVector, preamble);
  m_channel->Send (this, packet, GetPowerDbm (txVector.GetTxPowerLevel ()) + m_txGainDb,
                   txVector, preamble, mpdutype, frameDuration);
...
YansWifiPhy::SendTrnField (WifiTxVector txVector, uint8_t fieldsRemaining)
{
  ...
  m_channel->SendTrn (this, GetPowerDbm (txVector.GetTxPowerLevel ()) + m_txGainDb, txVector, fieldsRemaining);
}
...
YansWifiPhy::StartReceiveTrnField (WifiTxVector txVector, double rxPowerDbm, uint8_t fieldsRemaining)
{
  ...
  if (m_plcpSuccess)
    ...
    {
      NS_LOG_DEBUG ("Drop TRN Field because signal power too Small (" << rxPowerW << "<" << m_edThresholdW << ")");
    }
}
...
void 
YansWifiPhy::Configure80211ad (void)
{
  NS_LOG_FUNCTION (this);
  m_channelStartingFrequency = 56.16e3;
//  SetChannelWidth (1880); //1.88 GHz
  ...
  if (m_plcpSuccess == true)
    {
      ...
      if (m_psduSuccess)
      ...
      else
        {
          ...
          NS_LOG_DEBUG ("drop packet because the probability to receive it = " << rnd << " is lower than " << snrPer.per);
          NotifyRxDrop (packet);
          m_state->ReportPsduEndError (packet, snrPer.snr, isEndOfFrame);
        }
    }
  else
    {
