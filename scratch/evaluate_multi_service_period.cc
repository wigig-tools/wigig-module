 ...
 * Evaluate the allocation of multiple static service periods in the DTI access period.
 *
 * Network Topology:
 ...
 * ./waf --run "evaluate_multi_service_period"
 *
 * To run the script with different duration for the allocations e.g. SP1=10ms and SP2=5ms:
 * ./waf --run "evaluate_multi_service_period --sp1Duration=10000 --sp2Duration=5000"
 *
 * Simulation Output:
...
main (int argc, char *argv[])
{
  ...
  string dataRate3 = "10Mbps";                  /* Application Layer Data Rate for ApNode->WestNode. */
  ...
  string queueSize = "1000p";              /* Wifi MAC Queue Size. */
