 ...
 * This script is used to evaluate the performance of the IEEE 802.11ay protocol using custom SNR to BER lookup tables.
 * The tables are generated in MATLAB R2018b using WLAN Toolbox.
 *
 * Network Topology:
 * The scenario consists of a signle EDMG STA and a single EDMG PCP/AP.
 *
 *          EDMG PCP/AP (0,0)                       EDMG STA (+1,0)
 *
 ...
 *
 * Running Simulation:
 * ./waf --run "evaluate_edmg_error_model --simulationTime=10 --pcap=true"
 *
 * Simulation Output:
...
NS_LOG_COMPONENT_DEFINE ("EvaluateEdmgErrorModel");
...
void
PhyTxEnd (Ptr<const Packet> packet)
{
  WifiMacHeader hdr;
...
void
PhyRxDrop (Ptr<const Packet> packet)
{
  WifiMacHeader hdr;
  packet->PeekHeader (hdr);
  SnrTag tag;
  ...
  if (hdr.IsData ())
    {
      droppedPackets++;
//      snr+=RatioToDb(tag.Get());
    }
}

void
PhyRxEnd (Ptr<const Packet> packet)
{
  WifiMacHeader hdr;
  packet->PeekHeader (hdr);
  SnrTag tag;
...
{
  std::cout << "EDMG STA " << staWifiMac->GetAddress () << " associated with EDMG AP " << address
...
main (int argc, char *argv[])
{
  ...
  string phyMode = "EDMG_OFDM_MCS20";                 /* Type of the Physical Layer. */
...
//      LogComponentEnable ("EvaluateEdmgErrorModel", LOG_LEVEL_ALL);
      LogComponentEnable ("DmgErrorModel", LOG_LEVEL_ALL);
      LogComponentEnable ("DmgErrorModel", LOG_PREFIX_ALL);
    }
  ...
  wifi.SetRemoteStationManager ("ns3::IdealDmgWifiManager");
  ...
  std::cout << "  Average SNR: " << RatioToDb(snr / double(receivedPacketsMac)) << std::endl;
  AsciiTraceHelper ascii;
  Ptr<OutputStreamWrapper> outputPhy = ascii.CreateFileStream ("Evaluate_" + phyMode + ".csv", std::ios::app);
  *outputPhy->GetStream () << RatioToDb(snr / double(receivedPacketsMac)) << "," << double (receivedPackets) / double (transmittedPackets) << std::endl;
//  Ptr<OutputStreamWrapper> outputMac = ascii.CreateFileStream ("Evaluate_mac" + phyMode + ".csv", std::ios::app);
//  *outputMac->GetStream () << snr / double(receivedPacketsMac) << "," << double (packetSink->GetTotalReceivedPackets ()) / double (onoff->GetTotalTxPackets ()) << std::endl;
