 ...
 * service period for direct communication without going throught the DMG PCP/AP. The SP over multiple allocation blocks.
 *
...
uint16_t spDuration = 8000;                       /* The duration of a single SP allocation in MicroSeconds */
uint16_t spPeriod = 4000;                         /* The duration between consectuve SP allocations in MicroSeconds */
  ...
  if (accessPeriod == CHANNEL_ACCESS_DTI)
    {
      ...
      if ((rdsMac->GetAddress () == staWifiMac->GetAddress ()) &&
      ...
      else if ((srcRedsMac->GetAddress () == staWifiMac->GetAddress ()) && (dstRedsMac->GetAddress () == address))
        {
          ...
          std::cout << "Allocating static service period allocation for communication between " << srcRedsMac->GetAddress ()
                    << " and " << dstRedsMac->GetAddress () << std::endl;
          NS_ASSERT_MSG ((cbapDuration + spDuration * spBlocks + spPeriod * (spBlocks - 1)) < apWifiMac->GetDTIDuration (),
                         "Allocations cannot exceed DTI period");
          startTime = apWifiMac->AddAllocationPeriod (1, SERVICE_PERIOD_ALLOCATION, true,
                                                      srcRedsMac->GetAssociationID (), dstRedsMac->GetAssociationID (),
                                                      startTime, spDuration, spPeriod, spBlocks);
        }
    }
}
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("spPeriod", "The duration between consectuve SP allocations in MicroSeconds", spPeriod);
  ...
  /* Check values */
  NS_ASSERT_MSG ((firstPeriod + secondPeriod <= spDuration), "Relay First Period + Relay Second Period should be <= SP Duration");
